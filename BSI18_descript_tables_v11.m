...
% Original performance curve for the manscript before the Sports Medicine
% Revsion - 11-4-2024
[X_auc, Y_auc, ~,AUC]  = perfcurve(g, ypred2 ,1);

% The reviewer asked for error bars/ confidence limits.  We used
% the documentation from https://www.mathworks.com/help/releases/R2024b/stats/perfcurve.html#bunsogv-T
% The documentation for perfcurve shows how to do boot strapped confidence intervals
% which is what we implemented here
[X_auc2,Y_auc2,~,AUC2] = perfcurve(g, ypred2 ,1,'NBoot',1000,'XVals',[0:0.05:1]);

% Test code - SAVE incase we need to revist this topic, 11-4-24
%{
figure
hold on
plot(X_auc2(:,1),Y_auc2(:,1),'b-')
plot(X_auc2(:,1),Y_auc2(:,2),'r-')
plot(X_auc2(:,1),Y_auc2(:,3),'g-')

figure
errorbar(X_auc2(:,1),Y_auc2(:,1), Y_auc2(:,1)-Y_auc2(:,2), Y_auc2(:,1)-Y_auc2(:,2))
%}


% 11-4-24,  In order to make transparent error bars, create a filled pathc
% of vertexes.  - SAVE - 
new_x = vertcat(X_auc2(1:end-1,1),flip(X_auc2(2:end,1)) );
new_y = vertcat(Y_auc2(1:end-1,2),flip(Y_auc2(2:end,3)) );
%figure
%patch(new_x,new_y,'blue')
