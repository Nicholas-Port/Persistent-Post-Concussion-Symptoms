...
for s = 1:length(sub)
   ...
   for b = 1:sub(s).n_baseline
      ...
      if strcmp(sub(s).base(b).scat3completestatus,'Complete') == 1
         ...
         % % Oct 28, need to count the number of people with specific disorders
         ...
         % Oct 28, need to count the number of people with specific disorders
         % Creating a Neurological Disease table
         ND{ii,1}  = store_data(sub(s).base(b).menningitis          );
         ND{ii,2}  = store_data(sub(s).base(b).seizdis              );
         ND{ii,3}  = store_data(sub(s).base(b).sleepdis             );
         ND{ii,4}  = store_data(sub(s).base(b).balancedisorder      );
         ND{ii,5}  = store_data(sub(s).base(b).vestibulardisorder   );
         ND{ii,6}  = store_data(sub(s).base(b).vertigo              );
         ND{ii,7}  = store_data(sub(s).base(b).motionsickness       );
         ND{ii,8}  = store_data(sub(s).base(b).menieresdisease      );
         ND{ii,9}  = store_data(sub(s).base(b).balancedisorderother );
         ND{ii,10} = store_data(sub(s).base(b).modsevtbi            );
         ND{ii,11} = store_data(sub(s).base(b).brainsurgery         );
         ND{ii,12} = store_data(sub(s).base(b).stroke               );
         ND{ii,13} = store_data(sub(s).base(b).parkinsons           );
         ND{ii,14} = store_data(sub(s).base(b).memory               );
         ND{ii,15} = store_data(sub(s).base(b).alzheimers           );
         ND{ii,16} = store_data(sub(s).base(b).nonalzheimers        );
         ND{ii,17} = store_data(sub(s).base(b).mildcogimpariment    );
         ND{ii,18} = sub(s).base(b).age                              ;
   ...
   for in = 1:sub(s).n_injuries
      ...
      if ~isempty(temp_tp)
         if strcmp(sub(s).injury(in).time_p(temp_tp).scat3completestatus,'Complete') == 1           
            ...
            ND3{jj,1}  = store_data(sub(s).base(1).menningitis          );
            ND3{jj,2}  = store_data(sub(s).base(1).seizdis              );
            ND3{jj,3}  = store_data(sub(s).base(1).sleepdis             );
            ND3{jj,4}  = store_data(sub(s).base(1).balancedisorder      );
            ND3{jj,5}  = store_data(sub(s).base(1).vestibulardisorder   );
            ND3{jj,6}  = store_data(sub(s).base(1).vertigo              );
            ND3{jj,7}  = store_data(sub(s).base(1).motionsickness       );
            ND3{jj,8}  = store_data(sub(s).base(1).menieresdisease      );
            ND3{jj,9}  = store_data(sub(s).base(1).balancedisorderother );
            ND3{jj,10} = store_data(sub(s).base(1).modsevtbi            );
            ND3{jj,11} = store_data(sub(s).base(1).brainsurgery         );
            ND3{jj,12} = store_data(sub(s).base(1).stroke               );
            ND3{jj,13} = store_data(sub(s).base(1).parkinsons           );
            ND3{jj,14} = store_data(sub(s).base(1).memory               );
            ND3{jj,15} = store_data(sub(s).base(1).alzheimers           );
            ND3{jj,16} = store_data(sub(s).base(1).nonalzheimers        );
            ND3{jj,17} = store_data(sub(s).base(1).mildcogimpariment    );
            ND3{jj,18} = sub(s).base(1).age                              ;
            ...
            T3{jj,47} = sub(s).injury(in).injurymilsporttype_n;
            T3{jj,48} = sub(s).injury(in).time_p(temp_tp).scatday_from_injury;
...
ND2= cell2table(ND,'VariableNames', ...
    {'menningitis','seizdis','sleepdis','balancedisorder','vestibulardisorder',...
    'vertigo','motionsickness','menieresdisease','balancedisorderother',...
    'modsevtbi','brainsurgery','stroke','parkinsons','memory','alzheimers',...
    'nonalzheimers','mildcogimpariment','age' });

ND4= cell2table(ND3,'VariableNames', ...
    {'menningitis','seizdis','sleepdis','balancedisorder','vestibulardisorder',...
    'vertigo','motionsickness','menieresdisease','balancedisorderother',...
    'modsevtbi','brainsurgery','stroke','parkinsons','memory','alzheimers',...
    'nonalzheimers','mildcogimpariment','age' });
...
T2= cell2table(T,'VariableNames', ...
 ...
 T4= cell2table(T3,'VariableNames', ...
    ...
    'scatnausea','scatneckpain','scatnervanx','scatpresshead', ...
    'scatsad','scatsenslight','scatsensnois','scatsxsev_score',...
    'scattotnumsx_score','migraine','LOC','ncaa','sport2','site', 'situation',...
    'daysreportedlate', 'stimulant','injurymilsporttype','day_from_injury' });         
