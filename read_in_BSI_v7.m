...
%
disp('Read table 1');
T1 = readtable('/Users/laurenwilkins/Thesis/CARE_20/CARE_Baselines_2021_09_29_data_ran_2021_10_05.csv'              );
...
% START HERE AT BESS, plus fill in some things above
   ...
   % Figure out who is a cadet July 24, 2023  
   if strcmp(sub(s2).base(b).siteid{1}, 'NAVY')
      sub(s2).base(b).cadet = 1;
   elseif strcmp(sub(s2).base(b).siteid{1}, 'USAFA')
      sub(s2).base(b).cadet = 1;
   elseif strcmp(sub(s2).base(b).siteid{1}, 'USCGA')
      sub(s2).base(b).cadet = 1;
   elseif strcmp(sub(s2).base(b).siteid{1}, 'USMA')
      sub(s2).base(b).cadet = 1;
   else
      sub(s2).base(b).cadet = 0;
   end
...
disp('Read table 2');
T2 = readtable('/Users/laurenwilkins/Thesis/CARE_20/CARE_postinjury_assessments_NoControls_2021_09_29_data_ran_2021_10_05.csv');
...
for r = 1:size(T2,1)
   ...
   if T2{r,13} > 1
      ...
      sub(s).injury(ni).time_p(tp).siteid               = T2{r, 1  };
      sub(s).injury(ni).time_p(tp).injurydt             = T2{r, 8  };
      sub(s).injury(ni).time_p(tp).injurytime           = T2{r, 9  };
      sub(s).injury(ni).time_p(tp).inj_iuodate          = T2{r, 14  };
      sub(s).injury(ni).time_p(tp).inj_iutime           = T2{r, 15  };
      ...
      sub(s).injury(ni).time_p(tp).bsidays_from_injury = T2{r, 48 };
      ...
      sub(s).injury(ni).time_p(tp).sacdays_from_injury = T2{r, 82 };
      ...
      sub(s).injury(ni).time_p(tp).scatexamdate           = T2{r, 144 };
      sub(s).injury(ni).time_p(tp).scatstarttime          = T2{r, 145 };
      sub(s).injury(ni).time_p(tp).scatday_from_injury    = T2{r, 146 };
      ...
      % Figure out who is a cadet July 24, 2023  
      if strcmp(sub(s).injury(ni).time_p(tp).siteid{1}, 'NAVY')
         sub(s).injury(ni).time_p(tp).cadet = 1;
      elseif strcmp(sub(s).injury(ni).time_p(tp).siteid{1}, 'USAFA')
         sub(s).injury(ni).time_p(tp).cadet = 1;
      elseif strcmp(sub(s).injury(ni).time_p(tp).siteid{1}, 'USCGA')
         sub(s).injury(ni).time_p(tp).cadet = 1;
      elseif strcmp(sub(s).injury(ni).time_p(tp).siteid{1}, 'USMA')
         sub(s).injury(ni).time_p(tp).cadet = 1;
      else
         sub(s).injury(ni).time_p(tp).cadet = 0;
      end
...
% July 25, 2023 - trying the format auto feature to import some data which
% comes in as NAN
%T3 = readtable('../CARE_20/CARE_Injuries_NoControls_2021_09_29_data_ran_2021_10_05.csv','format','auto');
T3 = readtable('/Users/laurenwilkins/Thesis/CARE_20/CARE_Injuries_NoControls_2021_09_29_data_ran_2021_10_05.csv','format','auto');
disp('Read table 3 complete.')
...
for r = 1:size(T3,1)
   ...
   if ~isempty(ii)
      ...
      sub(s).injury(ii).siteid                    = T3{r,1 };
      sub(s).injury(ii).sport                     = T3{r,14};
      sub(s).injury(ii).injsituation              = T3{r,18};
      sub(s).injury(ii).injsport                  = T3{r,20};

      sub(s).injury(ii).days_reported_late        = T3{r,46};

      % Military injury types, etc
      sub(s).injury(ii).currmilsportlevel         = T3{r,42}; 
      sub(s).injury(ii).currmilclubsport          = T3{r,43}; 
      sub(s).injury(ii).currsport                 = T3{r,44}; 
      sub(s).injury(ii).injuryreportlate          = T3{r,45}; 
      sub(s).injury(ii).injurymilsporttype        = T3{r,50}; %which type of activity when injured: Club, PE class, Military Training, Outside of sport and military activities, Varsity/Intercollegiate Sports
      ...
      % Figure out who is a cadet July 24, 2023
      if strcmp(sub(s).injury(ii).siteid{1}, 'NAVY')
         sub(s).injury(ii).cadet = 1;
      elseif strcmp(sub(s).injury(ii).siteid{1}, 'USAFA')
         sub(s).injury(ii).cadet = 1;
      elseif strcmp(sub(s).injury(ii).siteid{1}, 'USCGA')
         sub(s).injury(ii).cadet = 1;
      elseif strcmp(sub(s).injury(ii).siteid{1}, 'USMA')
         sub(s).injury(ii).cadet = 1;
      else
         sub(s).injury(ii).cadet = 0;
      end

      % Possible values: Club, PE class, Military Training, Outside of sport and military activities, Varsity/Intercollegiate Sports
      if sub(s).injury(ii).cadet == 1

         if strcmp(sub(s).injury(ii).injurymilsporttype, 'Club')
            sub(s).injury(ii).injurymilsporttype_n = 1;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, 'PE class')
            sub(s).injury(ii).injurymilsporttype_n = 2;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, 'Military Training')
            sub(s).injury(ii).injurymilsporttype_n = 3;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, 'Outside of sport and military activities')
            sub(s).injury(ii).injurymilsporttype_n = 4;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, 'Varsity/Intercollegiate Sports')
            sub(s).injury(ii).injurymilsporttype_n = 5;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, 'Company Athletics/Intramurals')
            sub(s).injury(ii).injurymilsporttype_n = 6;
         elseif strcmp(sub(s).injury(ii).injurymilsporttype, '')

            t_str = sub(s).injury(ii).injsport{1};
            if strcmp( t_str(1:3), 'Out')
               sub(s).injury(ii).injurymilsporttype_n = 4;
            elseif strcmp( t_str(1:3), 'Var')
               sub(s).injury(ii).injurymilsporttype_n = 5;
            else
               keyboard
            end
         ...
         sub(s).injury(ii).injurymilsporttype_n = NaN;
